...
function CheckAuthentication()
{
	...
	session_start();
	if (!empty($_SESSION['CurrentManager']['ID'])) {
		return true;
	}
	else
	{
...
*/
// $baseUrl = 'http://'.$_SERVER['HTTP_HOST'].'/'.(preg_match('/\/(.+)\//U', $_SERVER['PHP_SELF'], $match) ? $match[1] : '').'/upload/ckfinder/';
$baseUrl = $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].(preg_match('/(.*)\/js\//', $_SERVER['PHP_SELF'], $match) ? $match[1] : '').'/upload/ckfinder/';
...
$config['ResourceType'][] = Array(
		...
		'maxSize' => 0,
		'allowedExtensions' => 'odt,odf,odp,doc,ppt,xls,txt,zip,rar,pdf,rtf,png,gif,jpg,ods,docx,xlsx,wmv',
		'deniedExtensions' => '');
